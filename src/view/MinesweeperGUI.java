...
public class MinesweeperGUI extends JPanel {
    ...
    public void showResultOverlay(OverlayType type, String title, String subtitle, int seconds) {
        ...
        OverlayStyle style = switch (type) {
            case GOOD -> OverlayStyle.POSITIVE;
            case BAD  -> OverlayStyle.NEGATIVE;
            default   -> OverlayStyle.INFO;
        };

        overlayTitle.setText(safeTitle);
        overlaySub.setText(safeSub);
        overlayIcon.setStyle(style);
    ...
    private void createOverlay() {
        ...
        msgCard.setPreferredSize(new Dimension(640, 220));
        ...
        centerRow.setBorder(BorderFactory.createEmptyBorder(6, 6, 6, 6));

        overlayIcon = new OverlayIcon();
        overlayIcon.setPreferredSize(new Dimension(72, 72));
        overlayIcon.setMaximumSize(new Dimension(72, 72));

        JPanel textCol = new JPanel();
        textCol.setOpaque(false);
        textCol.setLayout(new BoxLayout(textCol, BoxLayout.Y_AXIS));
        textCol.setBorder(BorderFactory.createEmptyBorder(0, 16, 0, 0));

        overlayTitle = new JLabel("", SwingConstants.LEFT);
        overlayTitle.setFont(new Font("Segoe UI", Font.BOLD, 42));
        overlayTitle.setForeground(Color.WHITE);

        overlaySub = new JLabel("", SwingConstants.LEFT);
        overlaySub.setFont(new Font("Segoe UI", Font.BOLD, 18));
        overlaySub.setForeground(new Color(220, 235, 255));
        overlaySub.setBorder(BorderFactory.createEmptyBorder(8, 0, 0, 0));

        textCol.add(overlayTitle);
        textCol.add(overlaySub);

        centerRow.add(Box.createHorizontalStrut(6));
        centerRow.add(overlayIcon);
        centerRow.add(textCol);
 ...
 private int countRevealedMines(Board board) {
    ...
    private class OverlayCardPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            OverlayStyle style = overlayIcon != null ? overlayIcon.getStyle() : OverlayStyle.INFO;

            g2.setColor(new Color(0, 0, 0, 90));
            g2.fillRoundRect(10, 12, w - 12, h - 12, 26, 26);

            g2.setColor(new Color(20, 35, 55, 238));
            ...
            g2.setColor(new Color(style.accent.getRed(), style.accent.getGreen(), style.accent.getBlue(), 220));
            g2.setStroke(new BasicStroke(6f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
            g2.drawLine(26, h - 18, w - 26, h - 18);
