...
public class MinesweeperGUI extends JPanel {
    ...
    // compute a good center-gift size based on overlay size
    ...
    /**
     * אוברליי תוצאה של שאלה (נכון/לא נכון) – עם theme כחול
     * ורק אחרי X שניות להריץ onDone (למשל endTurn).
     */
    public void showQuestionResultOverlayAndThen(
        ...
        showResultOverlay(type, title, subtitle, 0); // בלי טיימר פנימי
    ...
    private JPanel buildMainPanel() {
        ...
        // פנימי – רשת כפתורים
        JPanel innerA = buildSingleBoardPanel(board1, true);
        JPanel innerB = buildSingleBoardPanel(board2, false);

        // עם אנימציית המסגרת
        boardPanel1 = new AnimatedBoardPanel(innerA);
        boardPanel2 = new AnimatedBoardPanel(innerB);

        // matte frame
        boardWrapA = new BoardMattePanel(boardPanel1);
        boardWrapB = new BoardMattePanel(boardPanel2);
    ...
    private JPanel buildPlayersRow() {
        ...
        infoBtn.addActionListener(e -> {
            ...
            new GameRulesDialog(SwingUtilities.getWindowAncestor(this), currentDiff).setVisible(true);
        });
    ...
    private void startUiClock() {
        if (uiClockTimer != null && uiClockTimer.isRunning())
            uiClockTimer.stop();
    ...
    public void playGiftCenterAndShowOverlay(OverlayType type, String title, String subtitle, int overlaySeconds,
                                             Runnable onDone) {

        // נפילה – אין משאבים, אין גיפט במרכז
        if (overlayRoot == null || cardsHolder == null || giftOverlay == null
                || giftLabel == null || ICON_GIFT_CLOSED == null || ICON_GIFT_OPEN == null) {
        ...
        Timer t1 = new Timer(350, e1 -> {
            ...
            Timer t2 = new Timer(650, e2 -> {
                ...
                // טיימר חיצוני לסגירת האוברליי + מעבר תור
                if (onDone != null && overlaySeconds > 0) {
    ...
    // ========== refresh boards / state ==========

    private int countTotalMines(Board b) {
        int count = 0;
        for (int r = 0; r < b.getRows(); r++) {
            for (int c = 0; c < b.getCols(); c++) {
                if (b.getCell(r, c).getType() == CellType.MINE)
                    count++;
            }
        }
        return count;
    ...
    public void refreshView() {
        ...
        // mines left (אופציונלי: כמה מוקשים לא גלו עדיין)
        int total1 = countTotalMines(board1);
        int total2 = countTotalMines(board2);
        int revealed1 = countRevealedMines(board1);
        int revealed2 = countRevealedMines(board2);
        minesLeftALabel.setText("Mines left: " + Math.max(0, total1 - revealed1));
        minesLeftBLabel.setText("Mines left: " + Math.max(0, total2 - revealed2));
    ...
    // ⭐ כאן הטריגר למסגרת הזהב + האנימציה כשיש מעבר תור
    public void updateTurnHighlight() {
        ...
        if (boardPanel1 != null)
            boardPanel1.setActive(p1Active);
        if (boardPanel2 != null)
            boardPanel2.setActive(!p1Active);
    ...
    private enum OverlayStyle {
        ...
        OverlayStyle(Color accent) {
            this.accent = accent;
        }

        static OverlayStyle fromTitle(String title) {
            String t = (title == null) ? "" : title.toUpperCase();
            if (t.contains("GOOD") || t.contains("CORRECT") || t.contains("SUCCESS"))
                return POSITIVE;
            if (t.contains("BAD") || t.contains("WRONG") || t.contains("NOT") || t.contains("NO "))
                return NEGATIVE;
            return INFO;
        }
    }
            ...
            b.setBorder(
                    BorderFactory.createCompoundBorder(
                            BorderFactory.createLineBorder(
                                    isDark ? new Color(255, 255, 255, 60) : new Color(0, 0, 0, 40), 1, true),
    ...
    // ⭐ פאנל הלוח עם אנימציית "פתיחת תור":
    private static class AnimatedBoardPanel extends JPanel {
        ...
        private void drawWaveBand(Graphics2D g2, int x, int y, int w, int h, int arc, float progress) {

            int bandHeight = h / 3;
            int bandY = (int) (y - bandHeight + (h + bandHeight * 2) * progress);

            Shape oldClip = g2.getClip();
            Shape boardShape = new java.awt.geom.RoundRectangle2D.Float(x, y, w, h, arc, arc);
            ...
            GradientPaint gp1 = new GradientPaint(x, bandY, waveTop, x, bandY + bandHeight / 2, waveMid, true);
            g2.setPaint(gp1);
            g2.fillRect(x, bandY, w, bandHeight / 2);

            GradientPaint gp2 = new GradientPaint(x, bandY + bandHeight / 2, waveMid, x, bandY + bandHeight, waveBottom, true);
            g2.setPaint(gp2);
        ...
        private void drawTwoWayGoldBorder(Graphics2D g2, int x, int y, int w, int h, float progress) {
        ...
        private void drawBorderRange(Graphics2D g2, int x, int y, int w, int h, float tStart, float tEnd) {
        ...
        private void drawSparklesOnTwoWays(Graphics2D g2, int x, int y, int w, int h, float progress) {
            ...
            for (int i = 0; i < starCount; i++) {
                ...
                float alpha = 0.4f + 0.6f * (float) Math.abs(Math.sin(phase * 1.4f + i));
        ...
        private Point pointOnRectPerimeter(int x, int y, int w, int h, float t, float perimeter) {
    ...
    private void updateBoardView(Board board, CellButton[][] buttons, Color playerColor, boolean active) {
        ...
        // Glass look
        Color glass = tm.isDarkMode() ? new Color(255, 255, 255, 45) : new Color(255, 255, 255, 35);
        Color mineGlass = tm.isDarkMode() ? new Color(255, 120, 120, 85) : new Color(255, 140, 140, 70);
        Color questionGlass = tm.isDarkMode() ? new Color(140, 190, 255, 90) : new Color(160, 205, 255, 75);
        Color surpriseGlass = tm.isDarkMode() ? new Color(255, 170, 210, 90) : new Color(255, 185, 220, 75);
        Color usedGlass = tm.isDarkMode() ? new Color(255, 255, 255, 30) : new Color(255, 255, 255, 25);
    ...
    private static class BoardMattePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            Color bg = dark ? new Color(10, 18, 30, 160) : new Color(255, 255, 255, 170);
